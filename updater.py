...
Ported from status-portal's module of the same name (same author, same server,
same house style - see CLAUDE.md), scoped down for what this app actually has:
no `scheduler.py` here yet, so the periodic check is a plain background thread
rather than a registered scheduled task, and there's no 2FA to require step-up
for before the in-app "Update now" button runs - that button is gated the same
way every other admin POST in this app is (login + CSRF). Everything about the
*update itself* - the security posture below - is carried over unchanged.
...
Nothing here imports app.py, and nothing here touches Flask - so the CLI works on
a portal that won't even start.

--------------------------------------------------------------------------------
Security posture - read this before changing anything below
--------------------------------------------------------------------------------
Auto-update is the largest security surface this app has: it turns "someone got
into the admin panel" into persistent arbitrary code execution on the host. The
controls here are deliberate, and each one is load-bearing:

  * The repository is a **module constant**, never configurable - not from the
    admin UI, not from an env var, not from a CLI flag. A configurable update
    source would let anyone who can write a setting point the portal at their
    own "release".
  * Every request is HTTPS with certificate verification left on. Disabling
    certificate verification must never appear in this file.
  * Every URL actually fetched is re-validated against a host allow-list *after*
    the API hands it to us, because `browser_download_url` arrives over the
    network and is therefore untrusted input, not a constant.
  ...
  * The set of files replaced comes from the release archive's own member list -
    ...
    deny-list (`instance/`, `.env`) which aborts the whole update if a release
    archive ever contains one of those.
...
# Paths an update must never write to, whatever the archive claims. Gitignored,
# so cannot legitimately appear in a `git archive` zip - if one ever does, the
# release was built wrong and the update is aborted rather than trusted.
PROTECTED_PREFIXES = ("instance/",)
PROTECTED_FILES = (".env",)
...
# (an editor, an antivirus scanner, or this very app streaming a static file). Not
# a thing on POSIX, where a rename over an open file always succeeds - but
# harmless to retry there too, so there's one code path rather than an os.name
# branch.
...
class UpdateError(Exception):
    """Any expected, explainable failure - a network error, a bad archive, a
    refused precondition. Raised with a message meant to be shown to the admin
    verbatim."""
...
# ---------------------------------------------------------------------------
# Channel and check-enabled (routine admin toggles -> DB settings, per this
# project's config split)
# ---------------------------------------------------------------------------
def _read_setting(key, default):
    ...
    reach for when things are broken, including possibly the database itself,
    and `sqlite3.connect()` silently *creates* an empty file for a path that
    doesn't exist. Running `update.py check` on a fresh install must not leave a
    stray zero-table portal.db behind for init_db() to trip over later."""
...
def update_check_enabled():
    """Whether the portal polls GitHub for releases on its own. Off means the
    About page only ever checks when the admin presses "Check now" - for someone
    who would rather their home server not make a periodic outbound call."""
    return _read_setting("update_check_enabled", "1") != "0"


def set_update_check_enabled(enabled):
    if not os.path.isfile(db.DB_PATH):
        raise UpdateError("No database yet - start the portal once first.")
    db.set_setting("update_check_enabled", "1" if enabled else "0")
...
def _release_asset(release):
    """The release zip built by the release process (`games-portal-vX.Y.Z.zip`).

    Falls back to the tag's auto-generated zipball if a release has no uploaded
    asset. The fallback is still pinned to a *tag* in this repo (so it still has
    a version identity, which is what the whole check/compare/rollback story
    depends on), but it carries no published size or digest, so integrity
    verification is weaker for it - see `_download_asset`."""
...
def _normalise_release(release, channel):
    ...
    `body` is the changelog written at release time. It arrives from the
    network, so every consumer must treat it as untrusted text: the About page
    escapes it before display. Never hand it to anything that emits raw HTML."""
    return {
...
def fetch_releases(channel=None):
    """Every usable release on the given channel, newest first *by parsed
    version*.

    stable   -> non-prerelease GitHub releases only.
    unstable -> prereleases too (this project's own `-rc.N` releases).

    Sorting by version rather than publish date is the same decision documented
    on fetch_latest_release() below, applied to the whole list: republishing an
    old release must never look like an update, and a prerelease of an older
    line must never outrank a newer stable one."""
    ...
    try:
        response = requests.get(
            ...
            headers={"Accept": "application/vnd.github+json",
                     "User-Agent": f"games-portal/{current_version()}"},
...
def fetch_latest_release(channel=None):
    ...
    Note this picks by *version*, not by publish date: republishing an old
    release must never look like an update, and a prerelease of an older line
    must never outrank a newer stable one."""
    return fetch_releases(channel)[0]


# How many intervening releases the About page will list notes for. A cap only
# so a portal left un-updated for a long time doesn't render an unbounded page;
# the API fetch above is capped at 30 releases regardless.
MAX_RELEASE_NOTES = 20


def _release_notes_between(releases, current):
    """The releases strictly newer than `current`, newest first, plus the entry
    for the version being run if it happens to be on this channel."""
...
def check_for_update(channel=None):
    """Compares the running version against the latest on the channel. Never
    raises for a network failure - returns an `ok: False` result with an
    `error` string, so every caller (About page included) degrades to
    "couldn't check" instead of breaking."""
    channel = channel or get_channel()
    result = {
        "ok": False,
        "error": None,
    ...
    result.update({
        ...
        # Running something newer than anything published on this channel -
        # normal on a dev checkout, and on a stable-channel box handed a
        # prerelease. Reported as its own state rather than flattened into "up
        # to date", which would be misleading.
        "ahead": comparison > 0,
    })
    return result
...
# ---------------------------------------------------------------------------
# Cached update check (read by request handlers - never checked inline, per
# CLAUDE.md's "No slow I/O in a request handler" rule)
# ---------------------------------------------------------------------------
_update_cache = {"result": None, "refreshed_monotonic": None}


def get_cached_update_status():
    """Reads the cache only. None means nothing has been checked yet this
    process."""
...
def clear_update_cache():
    ...
    misleading next to the newly-selected one). The next
    refresh_update_cache_if_stale() call re-asks GitHub."""
...
# ---------------------------------------------------------------------------
# Background checker - a plain daemon thread rather than a registered
# scheduled task, since this app has no scheduler.py yet (unlike status-portal,
# where this is `scheduler.register(TASK_NAME, ...)`). If a scheduler ever gets
# built here for something else, this should move onto it instead of staying a
# bespoke loop.
# ---------------------------------------------------------------------------
...
def start_background_checker():
    """Starts the daemon thread once per process. Safe to call more than once -
    a second call is a no-op - so app.py and serve_waitress.py can both call it
    unconditionally at startup without coordinating."""
    global _checker_started
    with _checker_lock:
        if _checker_started:
            return
        _checker_started = True
    threading.Thread(target=_checker_loop, daemon=True, name="update-checker").start()


def _checker_loop():
    while True:
        try:
            refresh_update_cache_if_stale()
        except Exception:
            _logger.exception("Background update check failed")
        time.sleep(min(config.UPDATE_CHECK_INTERVAL_SECONDS, 3600))
...
def _download_asset(asset, progress):
    """Downloads the release archive into memory and verifies it before a
    single byte is written anywhere near the app directory.

    Three independent checks:
      1. HTTPS with certificate verification (never disabled), and the URL's
         host re-validated against ALLOWED_DOWNLOAD_HOSTS both before the
         request and after redirects have been followed.
      2. A hard byte cap while streaming, so a hostile or runaway response
         can't fill the disk.
      3. The downloaded bytes checked against the size and (when GitHub
         publishes one) the SHA-256 digest the releases API declared for that
         asset.
    ...
    size and digest come from the same API as the bytes themselves, so whoever
    can publish a release can publish matching metadata; this is an integrity
    check on the transfer, not an authenticity check on the publisher. It
    equally does not protect against an attacker who already controls the
    admin panel - see config.ENABLE_INAPP_UPDATE for the control that
    addresses that one."""
    url = asset["url"]
    ...
    try:
        ...
        response.raise_for_status()
        # requests followed any redirects itself; the URL that actually served
        # the bytes is the one that matters, so it gets the same host check.
    ...
    if expected_digest.startswith("sha256:"):
        ...
        # The zipball fallback publishes neither. Say so plainly instead of
        # letting "verified" be assumed - the transfer is still TLS-protected
        # and tag-pinned, there is just nothing independent to compare the
        # bytes against.
...
def _archive_members(zf):
    ...
    Handles both archive shapes this project can produce: `git archive` (files
    at the root, which is what the release process builds) and GitHub's
    auto-generated zipball (everything under one `owner-repo-sha/` directory,
    stripped here).

    Every path is validated: no absolute paths, no drive letters, no `..`,
    nothing that escapes the app directory (zip-slip), and nothing under a
    protected path. The release zip is our own and none of these should ever
    trigger - which is exactly why they abort the update loudly instead of
    ...
    # Strip a single common top-level directory if (and only if) every member
    # shares it - true for a GitHub zipball, false for a `git archive` zip.
    # "" and ".." are excluded as candidates even when they technically satisfy
    # "every member shares this first segment" (a single-entry archive made
    # entirely of "/etc/passwd" or "../evil.py" shares exactly one first
    # segment - "" or ".." respectively) - a real zipball's shared directory is
    # always a plain name like "owner-repo-<sha>", never one of those two. Get
    # this wrong and an absolute or parent-escaping path gets its dangerous
    # prefix silently stripped away *before* the checks below ever see it,
    # which defeats their entire purpose even though the final
    # destination-must-be-under-APP_ROOT check two guards down still catches
    # the write itself - this is about aborting loudly on a malformed archive,
    # not silently coercing it into something that happens to land safely.
    ...
    for info in infos:
        ...
        if not name:
            continue
        # Checked before any further normalisation - in particular, before a
        # leading "/" could be silently dropped and turn an absolute path into
        # a relative one that reads as fine to every check after this point.
        # ":" anywhere in the first segment is rejected too, not just a
        # Windows-style "C:" drive prefix - a colon is reserved/invalid in a
        # real filename on Windows and never appears in one of this project's
        # own release archives either way, so treating any occurrence as
        # suspicious costs nothing.
...
def _version_from_archive(zf, members):
    """Reads the VERSION file out of the archive - the incoming release's own
    idea of what it is, without importing any of its code."""
...
def _atomic_write(destination, data, mode=None):
    ...
    leaves the old file intact rather than a truncated one. On Windows the
    rename itself can still fail while another process holds the destination
    open (an editor, an antivirus scanner, or this app streaming a static file
    to a browser), hence the retry - and if it still fails, the caller rolls
    the whole update back rather than leaving a half-updated tree."""
...
def _make_backup(members, from_version, to_version, channel):
    """Copies every file that is about to be overwritten into a timestamped
    folder under instance/ (which updates never touch, so a backup can't be
    clobbered by the next one), plus a manifest recording which files were
    *added* so a rollback can remove them again rather than leaving them
    behind."""
...
def rollback(backup_name=None, progress=None):
    """Restores a backup over the app directory: replaced files copied back,
    added files deleted. Defaults to the most recent backup.

    This is the recovery path for the one failure mode nothing inside the app
    can handle on its own - see `write_pending_marker()` for why."""
...
def write_pending_marker(backup_name, to_version):
    """Records "an update was just applied and the app is about to restart into
    it".

    Be clear-eyed about what this does: it lets the *next successful start*
    confirm it came up on the new version, and it records which backup to roll
    back to. It cannot detect a failed start. Once os.execv replaces the process
    image, no code from the old version exists any more - if the new code dies
    on import, nothing of ours is running to notice or to undo it. That is a
    property of in-place restart, not an oversight: genuine automatic
    post-restart rollback requires a supervisor *outside* this process, which
    this project deliberately doesn't ship because it would change how everyone
    launches the portal.

    So the honest split is:
      * every failure BEFORE the restart (download, verification, a file that
        won't replace, a failed dependency install) is rolled back
        automatically, in process, by perform_update();
      * the failure AFTER the restart is recovered manually and reliably with
        one command over SSH: `python update.py rollback`, which reads this
        marker to know exactly which backup to restore."""
    try:
...
def check_pending_marker():
    ...
    worked - log it and clear the marker. If it doesn't, the process came back
    on the old code (a partially applied update, or a launcher pointing
    somewhere else): the marker is left in place and the problem logged loudly,
    so the admin finds it in the logs and `update.py rollback` still knows
    which backup to use."""
...
def _install_dependencies(progress):
    ...
    precisely because it happens before the restart - the caller rolls the code
    back rather than restarting into a version whose dependencies aren't
    installed."""
...
def perform_update(channel=None, force=False, install_deps=True, allow_dev_checkout=False,
    ...
    UpdateError with an explainable message on any refused precondition or
    failure.

    Safe to run twice and safe to run when already up to date: the version
    check short-circuits with `applied: False` unless `force` is set, and the
    whole file-replacement step is idempotent (the same archive written twice
    produces the same tree).

    Does NOT restart anything - that's the caller's call, because the CLI can't
    know how the portal is launched and the web route wants to use the app's
    own existing in-place restart."""
    progress = progress or _default_progress
    channel = channel or get_channel()
    ...
    with zf:
        ...
        try:
            ...
            # A half-updated tree is worse than either version, so any failure
            # part way through unwinds everything before returning.
