...
the admin panel's "Update now" button calls too, so the two can never drift
apart.

It deliberately does not import app.py or Flask - it is the tool you reach for
precisely when the web UI is broken, so it must work on a portal that won't
start.
...
your secret key, your update backups) and .env.
"""
import argparse
import json
import os
...
# Mirrors updater.CHANNELS. Duplicated as a literal purely so argparse can be
# built without importing the app - see _load_updater() below.
...
def _load_updater():
    ...
    Deliberately lazy rather than a module-level import: an update that goes
    wrong can leave a tree whose own modules no longer import (a half-applied
    set of files, a config.py from a release that predates something
    updater.py needs). That is precisely when someone runs this script - so it
    must get far enough to reach the emergency rollback below instead of dying
    on an ImportError."""
    try:
        import updater
        return updater
...
def _emergency_rollback(backup_name=None):
    ...
    This is NOT a second update implementation - it only restores a backup
    that updater.py already created, by reading the manifest.json it already
    wrote. It exists because the normal path (updater.rollback) needs
    updater.py to import, and the one failure mode a rollback tool most needs
    to survive is "the update broke this app's code". Everything it does could
    equally be done by hand with `cp -r instance/update_backups/<name>/* .` -
    it just does it correctly, including removing files the update added."""
...
def cmd_apply(args):
    ...
    if not args.yes and not _confirm(
            f"Update this portal to {result['latest']}? Your database, .env and update backups "
            "are never touched."):
        print("Cancelled.")
        return 1
    print()
...
def build_parser():
    parser = argparse.ArgumentParser(
        prog="update.py",
        description="Update games-portal to the latest release from its GitHub "
...
def main(argv=None):
    ...
    if not getattr(args, "command", None):
        # Bare `python update.py` is the thing you type when you just want to
        # know where you stand, so it reads rather than writes.
        args = parser.parse_args(["check"])
    # ASCII only, deliberately: this script has to run in whatever console the
    # machine has, including an old Windows cmd.exe on codepage 437 where a
    # stray em dash raises UnicodeEncodeError. A recovery tool must not fail on
    # cosmetics.
    print(f"games-portal updater - app directory: {APP_ROOT}\n", flush=True)
